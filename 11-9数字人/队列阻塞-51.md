...
- ⚠️ **问题**: 虽然 GPU 可用，但利用率为 0%

#### 可能原因
1. **ONNX Runtime 未明确使用 GPU**
   - 虽然 CUDAExecutionProvider 可用，但可能未在代码中使用
   - `service/trans_dh_service` 是编译后的 .so 文件，无法直接修改
   - 需要通过环境变量或其他方式配置
...
# 注意：ORT_EXECUTION_PROVIDERS 环境变量可能无效
# 需要在代码中明确指定提供者
```
...
#### 为什么有效？
- **确保使用 GPU**: 虽然无法直接修改 .so 文件，但可以通过环境变量优化
- **减少多进程冲突**: 禁用并行，避免冲突
- **优化配置参数**: 提高性能
