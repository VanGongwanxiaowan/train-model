...
根据对日志 `dh.log` (167-179行) 的深入分析和 GitHub 仓库的研究，队列阻塞问题的根本原因是：

### 1. PyTorch DataLoader 多进程死锁 🔴

**关键问题**:
- `landmark2face_wy/checkpoints/test/opt.txt` 中 `num_threads: 4`
- DataLoader 使用 `num_workers=int(opt.num_threads)=4`
- 在多进程环境下，DataLoader 的多工作进程与 CUDA 结合时会导致死锁
- 上游队列快速发送数据（10个数据包在1秒内完成）
- 下游队列完全没有消费数据（60秒内没有任何处理）
- 导致上游队列满，阻塞
...
- 共享内存: 64MB（太小，但对于单进程模式足够）
- 有多个僵尸进程（defunct processes）

## 已实施的修复方案

### ✅ 修复 1: 修改模型配置中的 num_threads（最关键）
...
### ✅ 修复 3: ONNX Runtime 版本降级

**版本**: `onnxruntime-gpu==1.16.0`（项目推荐版本）

**原因**: 
- 项目文档推荐使用 1.16.0
- 1.18.0 在多进程环境下可能存在 CUDA 同步问题
...
- ✅ **队列阻塞问题解决**: DataLoader 使用单进程模式，避免多进程死锁
- ✅ **处理速度提升**: 队列能够及时处理数据，不再阻塞
- ✅ **GPU 利用率提升**: GPU 不再空闲等待，利用率 > 0%
...
如果出现模型加载错误（如 `RuntimeError: PytorchStreamReader failed reading zip archive`），可能是模型文件损坏，需要重新下载。
